...
const registerUser = async (req, res, next) => {
  ...
  if (!name || !email || !password || !phonenumber || !role||!address) {
  ...
  try {
      ...
      const user = await User.create({ name, email, password, phonenumber, role,address });
      ...
      return res.status(201).json({ name, email, role, token, password, phonenumber,address });
  } catch (error) {
      next(error);
  }
};
